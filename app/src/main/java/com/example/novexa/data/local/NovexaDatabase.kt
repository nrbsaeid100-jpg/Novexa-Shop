...
abstract class NovexaDatabase : RoomDatabase() {
    ...
    companion object {
        @Volatile
        private var INSTANCE: NovexaDatabase? = null

        fun getDatabase(context: Context): NovexaDatabase {
            return getDatabase(context, CoroutineScope(Dispatchers.IO))
        }

        fun getDatabase(context: Context, scope: CoroutineScope): NovexaDatabase {
            return INSTANCE ?: synchronized(this) {
                val instance = Room.databaseBuilder(
                    context.applicationContext,
                    NovexaDatabase::class.java,
                    "novexa_ecommerce_db"
                )
                    .addCallback(NovexaDatabaseCallback(scope))
    ...
    private class NovexaDatabaseCallback(
        ...
        private suspend fun populateInitialData(db: NovexaDatabase) {
            // 1. Seed Users
